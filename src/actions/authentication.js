...
//server not accept json data so i use qs (form data urlencode)
   const auth= qs.stringify({email,password})
    axios.post('https://chinhanh.herokuapp.com/api/user/login',auth ,{ headers: {
...
export  const secretUser = (password) => dispatch => {
    //server not accept json data so i use qs (form data urlencode)
       const auth= qs.stringify({password})
        axios.post('https://chinhanh.herokuapp.com/api/user/secret',auth ,{ headers: {
            'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'
          }}
      )
...
export const getAll=()=>{
    ...
    return fetch('https://chinhanh.herokuapp.com/api/user',requestOptions).then(
        res=>{ return res.data}
    )
}